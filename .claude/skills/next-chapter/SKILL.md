...
- Được phép tự sáng tạo thêm nhân vật/phe phái/hệ thống/địa danh mới nếu mạch truyện cần, miễn là không mâu thuẫn với lore đã có — nhớ ghi lại ở Bước 5
...
6. Nếu chương vừa viết giới thiệu nhân vật/phe phái/hệ thống/địa danh MỚI chưa từng có trong lore, tạo ngay hồ sơ tương ứng: nhân vật mới → file riêng trong `<story>/characters/` dựa trên `_template.md` + thêm dòng vào `_index.md`; phe phái/hệ thống/địa danh mới → thêm dòng vào `<story>/lore/factions.md` / `systems.md` / `geography.md`. Không được để thông tin mới chỉ nằm trong văn xuôi chương.
