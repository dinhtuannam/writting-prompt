# Danh Sách Nhân Vật

| Tên | Vai trò | Trạng thái | File chi tiết |
|---|---|---|---|
| Ronan | Nhân vật chính | Bị giam, bị cưỡng ép dùng thuốc an thần | `ronan.md` |
| Rodrick | Phụ (cha) | Bị bắt giữ, chờ thụ án | `rodrick.md` |
| Maren | Phụ (mẹ) | Bị bắt giữ, chờ thụ án | `maren.md` |
| Lãnh Chúa Arik | Phản diện chính | Còn sống | `arik.md` |
| Freya | Nữ chính (con gái Arik, mang Ấn Rồng) | Còn sống, an toàn | `freya.md` |

> Cập nhật bảng này mỗi khi thêm nhân vật mới. Mỗi nhân vật quan trọng có 1 file riêng trong thư mục này, dùng `_template.md` làm khuôn mẫu (đổi tên thành `<ten-nhan-vat>.md`).
